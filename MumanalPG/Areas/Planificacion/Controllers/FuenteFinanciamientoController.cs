...
namespace MumanalPG.Areas.Planificacion.Controllers
{
    ...
    {        
        ...
        [Breadcrumb("FuenteFinanciamiento", FromController = "Dashboard", FromAction = "Clasificadores")]
        public async Task<IActionResult> Index(string filter, int page = 1, string sortExpression = "Descripcion", string a = "")
        { 
            var consulta = DB.FuenteFinanciamiento.AsNoTracking().AsQueryable();
            consulta = consulta.Where(m => m.IdEstadoRegistro != Constantes.Eliminado); // != el estado es diferente a ANULADO
        ...
        {
            var item = await DB.FuenteFinanciamiento.FindAsync(id);
            item.IdEstadoRegistro = Constantes.Eliminado ;
